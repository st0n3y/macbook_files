	...
	*/
	...
	if( !function_exists('gdlr_print_portfolio_item') ){
		function gdlr_print_portfolio_item( $settings = array() ){
			...
			$ret .= '<div class="portfolio-item-wrapper type-' . $settings['portfolio-style'] . '" ' . $item_id . $margin_style . '>'; 
			...
			if( $settings['portfolio-filter'] == 'enable' ){
				...
				$ret .= 'data-orderby="' . $args['orderby'] . '" data-order="' . $args['order'] . '" data-num-excerpt="' . $settings['num-excerpt'] . '" ';
			...
			if($settings['portfolio-style'] == 'detail-portfolio'){		
				$ret .= gdlr_get_detail_portfolio($query, $settings['thumbnail-size'], $settings['num-excerpt']);
	...
	// print detailed portfolio
	if( !function_exists('gdlr_get_detail_portfolio') ){
		function gdlr_get_detail_portfolio($query, $thumbnail_size, $num_excerpt){
			global $post, $gdlr_excerpt_read_more;
			
			$ret = '';
			$gdlr_excerpt_read_more = false; 	
			if( !empty($num_excerpt) && $num_excerpt < 0 ){
				global $gdlr_excerpt_length; $gdlr_excerpt_length = $num_excerpt;
				add_filter('excerpt_length', 'gdlr_set_excerpt_length');
			} 				
			while($query->have_posts()){ $query->the_post();
				$ret .= '<div class="gdlr-item gdlr-portfolio-item gdlr-detail-portfolio">';
				$ret .= '<div class="gdlr-ux gdlr-detaul-portfolio-ux">';
				...
				if( $num_excerpt < 0 ){
					$ret .= '<div class="portfolio-content">' . gdlr_content_filter(get_the_content(''), true) . '</div>';
				}else{
					$ret .= '<div class="portfolio-content">' . get_the_excerpt() . '</div>';
				}
				
				// portfolio button
				$ret .= '<a class="gdlr-button portfolio-continue-reading" href="' . get_permalink() . '">';
				$ret .= __('Read More' ,'gdlr-portfolio');
				$ret .= '</a>';
				if(!empty($port_option['website'])){
					$ret .= '<a class="gdlr-button portfolio-visit-website" target="_blank" href="' . $port_option['website'] . '">';
					$ret .= __('Visit Website' ,'gdlr-portfolio');
					$ret .= '</a>';
				}
	...
	if( !function_exists('gdlr_get_classic_carousel_portfolio') ){
		function gdlr_get_classic_carousel_portfolio($query, $size, $thumbnail_size){	
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= gdlr_get_portfolio_info(array('tag'), array(), false);
				$ret .= '</div>';
				$ret .= '</div>';
				$ret .= '</li>';
			}			
