<?php
	...
	if( !function_exists('gdlr_shortcode_content_filter') ){
		add_filter( 'gdlr_shortcode_content', 'wptexturize'        ); add_filter( 'gdlr_shortcode_content', 'convert_smilies'    );
		add_filter( 'gdlr_shortcode_content', 'convert_chars'      ); add_filter( 'gdlr_shortcode_content', 'wpautop'            );
		add_filter( 'gdlr_shortcode_content', 'shortcode_unautop'  ); add_filter( 'gdlr_shortcode_content', 'prepend_attachment' );	
		add_filter( 'gdlr_shortcode_content', 'do_shortcode', 11 	 );
		function gdlr_shortcode_content_filter( $content, $main_content = false ){
			if($main_content) return str_replace( ']]>', ']]&gt;', apply_filters('the_content', $content) );
			$content = str_replace("<br />", "\n", $content);
			return apply_filters('gdlr_shortcode_content', $content);
		}		
	}
	...
	function gdlr_button_shortcode( $atts, $content = null ){
		...
		$button_class  = $size;
		$button_class .= ($with_border != 'no')? ' with-border': ''; 
		return '<a class="gdlr-button ' . $button_class . '" href="' . $href . '" target="' . $target . '" ' . $style . ' >' . do_shortcode($content) . '</a>';
	}	
	...
	function gdlr_stunning_text_shortcode( $atts, $content = null ){
		...
		if(!empty($content)){
			...
			$ret .= '<div class="stunning-text-caption" ' . $stunning_style . '>' . gdlr_shortcode_content_filter($content) . '</div>';
		}
	...
	function gdlr_column_shortcode( $atts, $content = null ){
		...
		if( function_exists('gdlr_get_column_class') ){
			...
			$ret .= gdlr_shortcode_content_filter($content);
			$ret .= '</div>';
			$ret .= '</div>';
			return $ret;
		}
	...
	function gdlr_image_link_shortcode( $atts ){
		...
		$ret .= '<span class="gdlr-image-link-overlay">';
	...
	function gdlr_post_slider_shortcode( $atts, $content = null ){
		...
		$settings['orderby'] = $orderby;
		$settings['order'] = $order;
		$settings['slider-style'] = 'post-' . $caption_position . ' post-slider';
