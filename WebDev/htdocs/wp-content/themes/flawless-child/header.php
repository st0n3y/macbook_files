...
$env = strtoupper(substr($HOST,0,strpos($HOST,'.')));
?>
<!--<![endif]-->
...
<head>
	...
	<title><?php if ($env=='DEV' || $env == 'QA') { echo $env . ' :: '; } ?> <?php bloginfo('name'); ?>  <?php wp_title(); ?></title>
	...
	<?php
		...
		$dev_header = '<div class="' . strtolower($env) . '_header"><p align="center"><b>' . strtoupper(substr($HOST,0,strpos($HOST,'.'))) . '</b>: You are currently viewing the ' . (($env=='DEV')?'development':'QA') . ' environment</p></div>';
		$site_warning = '<div class="downtime_warning"><p align="center"><b>Warning:</b> CodeClan website will be unavailable due to scheduled maintenance on Thursday 28th July 2016 from 08:00 to 13:00 BST</p></div>';
                wp_head();
	?>

<script type='text/javascript'>window.Muscula = { settings:{
        logId:"qlugVpE-WbzQo", suppressErrors: false, googleAnalyticsEvents: 'none'
    }};
    (function () {
...
<div class="body-wrapper <?php echo $body_wrapper; ?>">
	...
	<header class="gdlr-header-wrapper <?php echo $header_class; ?>">
		...
		<!-- top navigation -->
		<?php if ($HOST == 'dev.codeclan.com' || $HOST == 'qa.codeclan.com') { echo $dev_header; } ?>
		...
		<div class="gdlr-header-substitute">
			...
			<!--Courses Sub Menu -->
                        ...
                        <?php if ( is_tree('12') && !in_array($pageID, array(1955, 2306, 2307, 2308)) ) : ?>
				<div class="secondary-nav">
				<?php wp_nav_menu( array( 'theme_location' => 'course-menu', 'container_class' => 'nav-secondary' ) ); ?>
				</div>
			<?php endif;?>
