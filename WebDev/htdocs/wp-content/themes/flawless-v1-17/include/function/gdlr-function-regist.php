...
<?php			
		...
		function gdlr_get_default_admin_option() {
			...
			if(empty($sidebar_option)){
				update_option('gdlr_sidebar_name', 
					array( 'blog', 'blog-left', 'portfolio', 'portfolio-left', 'shortcodes',  'woocommerce',  'Features', 'Archives')
				);
			}
			
			$google_font_list = get_option(THEME_SHORT_NAME . '_google_font_list', array());
			if( empty($google_font_list) ){
				$font_text  = 'a:1:{s:9:"Open Sans";a:2:{s:7:"subsets";a:7:{i:0;s:5:"greek";i:1;s:12:"cyrillic-ext";i:2;s:8:"cyrillic";i:3;s:5:"latin";i:4;s:9:"';
				$font_text .= 'latin-ext";i:5;s:10:"vietnamese";i:6;s:9:"greek-ext";}s:8:"variants";a:10:{i:0;s:3:"300";i:1;s:9:"300italic";i:2;s:7:"regular";i:';
				$font_text .= '3;s:6:"italic";i:4;s:3:"600";i:5;s:9:"600italic";i:6;s:3:"700";i:7;s:9:"700italic";i:8;s:3:"800";i:9;s:9:"800italic";}}}';
	...
	if( !function_exists('gdlr_excerpt_more') ){
		function gdlr_excerpt_more( $more ) {
			...
			return '... <a href="' . get_permalink() . '" class="excerpt-read-more">' . __( 'Read More', 'gdlr_translate' ) . '</a>';
		}
	}
	...
	if ( !function_exists('gdlr_comment_list') ){
		function gdlr_comment_list( $comment, $args, $depth ){
			...
			if( !empty($theme_option['new-fontawesome']) && $theme_option['new-fontawesome'] == 'enable' ){
				...
				$icon_class = array('date'=>'icon-time', 'reply'=>'icon-mail-reply');
			}			
	...
	<article id="comment-<?php comment_ID(); ?>" class="comment-article">
		...
		<div class="comment-body">
			...
			<div class="comment-reply">
				<?php comment_reply_link( array_merge($args, array('before' => ' <i class="' . $icon_class['reply'] . '"></i>', 'reply_text' => __('Reply', 'gdlr_translate'), 'depth' => $depth, 'max_depth' => $args['max_depth'])) ); ?>
			</div><!-- reply -->					
			</header>
...
<li class="gdlr-mega-menu">
	<?php 
		if(is_user_logged_in()){
			echo '<a href="' . wp_logout_url(get_permalink()) . '"><i class="fa ' . gdlr_fa_class('icon-lock') . '"></i>' . __('Logout', 'gdlr_translate') . '</a>';
			...
			echo '<a href="#"><i class="fa ' . gdlr_fa_class('icon-lock') . '"></i>' . __('Login', 'gdlr_translate') . '</a>';
	?>
	<div class="sf-mega">
...
?>
<li class="gdlr-mega-menu">
	<a href="#"><i class="fa <?php echo gdlr_fa_class('icon-envelope'); ?>"></i><?php echo __('Subscribe', 'gdlr_translate'); ?></a>
	<div class="sf-mega">
