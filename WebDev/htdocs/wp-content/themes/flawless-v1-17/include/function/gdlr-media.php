	...
	*/	
		...
		function gdlr_get_image_frame_item($settings){
			...
			if( !empty($settings['link-type']) && $settings['link-type'] != 'none' ){
				...
				if( $settings['link-type'] == 'url' ){
					...
					$icon = gdlr_fa_class('icon-search');
		...
		function gdlr_get_post_slider_item( $settings ){
			...
			if( !empty($settings['category']) ){
				$args['tax_query'] = array(
					array('terms'=>explode(',', $settings['category']), 'taxonomy'=>'category', 'field'=>'slug')
				);
			}
			...
			while($query->have_posts()){ $query->the_post();
				...
				if( !empty($image_id) ){
					...
					$slide_data[$image_id] = array(	
						...
						'url'=> get_permalink(),
						'new-tab'=> 'disable',
						'caption-position'=>$settings['slider-style']
					);
			...
			$ret  = '<div class="gdlr-item gdlr-post-slider-item style-' . $settings['slider-style'] . '" ' . $item_id . $margin_style . ' >';
	...
	if( !function_exists('gdlr_get_banner_item') ){
		function gdlr_get_banner_item( $settings ){
			...
			// banner description
			$ret  = '<div class="gdlr-banner-description three columns gdlr-nav-container">';
			$ret .= '<div class="gdlr-banner-description-item gdlr-item" ' . $item_id . $margin_style . ' >';
			if( !empty($settings['banner-title']) ){
				$ret .= '<h4 class="gdlr-banner-title">';
				$ret .= $settings['banner-title'];
				$ret .= '</h4>';
			}
			if( !empty($settings['banner-description']) ){
				$ret .= '<div class="gdlr-banner-description">';
				$ret .= $settings['banner-description'];
				$ret .= '</div>';
			}
			$ret .= '<div class="nav-container style-1" ></div>';
			$ret .= '</div>';
			$ret .= '</div>'; // gdlr-banner-description
			
			// banner images	
			$ret .= '<div class="gdlr-banner-images nine columns">';
			$ret .= '<div class="gdlr-banner-item gdlr-item" >';
