	...
	if( !function_exists('gdlr_get_blog_info') ){
		function gdlr_get_blog_info( $array = array(), $wrapper = true ){
			...
			if( !empty($theme_option['new-fontawesome']) && $theme_option['new-fontawesome'] == 'enable' ){
				$icon_class = array('date'=>'fa fa-clock-o', 'tag'=>'fa fa-tag', 'category'=>'fa fa-folder', 
					'comment'=>'fa fa-comment', 'author'=>'fa fa-user');
			}else{
				$icon_class = array('date'=>'icon-time', 'tag'=>'icon-tag', 'category'=>'icon-folder-close', 
					'comment'=>'icon-comment', 'author'=>'icon-user');
			}
